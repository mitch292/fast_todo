...
async def create_task(task: TaskInCreate, tasks_repo=Depends(TasksRepository),     current_user: UserInResponse = Depends(get_current_active_user)
):
...
async def get_task(id: UUID, tasks_repo=Depends(TasksRepository),     current_user: UserInResponse = Depends(get_current_active_user)
) -> TaskInResponse:
...
async def update_task(
    id: UUID, task: TaskInUpdate, tasks_repo=Depends(TasksRepository),     current_user: UserInResponse = Depends(get_current_active_user)
...
async def delete_task(id: UUID, tasks_repo=Depends(TasksRepository),     current_user: UserInResponse = Depends(get_current_active_user)
):
