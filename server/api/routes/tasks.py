...
async def create_task(task: TaskInCreate, tasks_repo = Depends(get_repository(TasksRepository))):
...
async def get_task(id: UUID, tasks_repo = Depends(get_repository(TasksRepository))) -> TaskInResponse:
    return await tasks_repo.get_task_by_id(id)

@task_router.put('/{id}', response_model=TaskInResponse)
async def update_task(id: UUID, task: TaskInUpdate, tasks_repo = Depends(get_repository(TasksRepository))):
